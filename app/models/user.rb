class User < ActiveRecord::Base
  ...
  # :confirmable, :lockable, :timeoutable and :omniauthable
    ...
    def pending_friend_requests_from
    	self.friendships.where(state:"pending").map(&:friend) + self.inverse_friendships.where(state:"pending").map(&:user)
    end
    ...
    def friendship_relation(user_2)
      Friendship.where(user_id: [self.id, user_2.id], friend_id: [user_2.id, self.id],
        friend_id: [user_2, self.id]).first
    end
end
